...
function handle(message, json) {
    if (json || typeof(message) === 'string') {
        ...
        const { ls, rm, download, gcmd, upload, connect, raw, md5 } = msg;
        if (ls) {
            carvera.ls(ls);
        } else if (rm) {
            carvera.rm(rm);
            ...
            log({ download, md5 });
            carvera.download(download, (data, md5, error) => {
                send({ file: download, data, md5, error });
            }, md5);
        } else if (gcmd !== undefined) {
            carvera.send(raw ? gcmd : `${gcmd}\n`);
